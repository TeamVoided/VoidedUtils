...
import org.teamvoided.voidlib.core.mc
...
class ModelProvider(output: FabricDataOutput) : FabricModelProvider(output) {
    ...
    override fun generateBlockStateModels(gen: BlockStateModelGenerator) {
        try {
            ...
            VUBlocks.TOGGLEABLE_BUTTONS.forEach { inheritBtn(it, it.btn.gId, gen) }
            ...
            stairs(ConsistentStones.CUT_SANDSTONE_STAIR, mc("sandstone_top"), Blocks.CUT_SANDSTONE.gId, gen)
            stairs(
                ConsistentStones.CUT_RED_SANDSTONE_STAIR, mc("red_sandstone_top"), Blocks.CUT_RED_SANDSTONE.gId, gen
            )
            ...
            fence(ConsistentStones.RED_NETHER_BRICK_FENCE, Blocks.RED_NETHER_BRICKS.gId.withPrefix("block/"), gen)
    ...
    ) = stairs(block, src, bottom.gId, side.gId, top.gId, gen)
    ...
    private fun slab(block: VoidedSlabBlock, bottom: Block, side: Block, top: Block, gen: BlockStateModelGenerator) =
        slab(
            block, Texture.texture(block.block)
                .put(TextureKey.BOTTOM, bottom.gId.withPrefix("block/"))
                .put(TextureKey.SIDE, side.gId.withPrefix("block/"))
                .put(TextureKey.TOP, top.gId.withPrefix("block/")), gen
        )
    ...
    fun slab(block: VoidedSlabBlock, texture: Texture, gen: BlockStateModelGenerator) {
        ...
        val id3 = block.block.gId.withPrefix("block/")
    ...
    fun wall(block: VoidedWallBlock, gen: BlockStateModelGenerator) = wall(block, block.block.gId, gen)
    ...
    private fun pressurePlate(plate: VoidedPressurePlateBlock, gen: BlockStateModelGenerator) =
        pressurePlate(plate, plate.block.gId, gen)
